(ns lime-scooters.main
  "Kotodama WASM entrypoint for the lime_scooters clean-room actor (L4) — Clojure port.
  ...
  facts (`lime_scooters.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "lime_scooters")
(def tier "L4")
...
(def entity-specs
  [{:entity "Driver"   :plural "drivers"   :id-prefix "limescoo_dri"
   ...
   {:entity "Rating"   :plural "ratings"   :id-prefix "limescoo_rat"
...
(defn emit-facts
  "EAVT facts for one record: {\"lime_scooters.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "lime_scooters-compat" :tier tier :entities entities} 200])
...
(defn start! [] :lime_scooters-compat/ready)
